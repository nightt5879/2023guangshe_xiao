...
class pathPlaner:
    ...
    def get_paths_list(self):
        ...
        for paths in self.paths:
            lst = [Dict['move_mode'] for Dict in paths['action']]
            merged_lst = []
            count = 1
            for i in range(1, len(lst)):
                if lst[i] == lst[i - 1]:
                    ...
                    merged_lst.append(lst[i - 1] + str(count))
                    count = 1
            merged_lst.append(lst[-1] + str(count))
            target_xy = paths['action'][-1]['target_xy']
    ...
    def update_paths(self):
        ...
        :return:
        """
        mine_x, mine_y, now_xy, car_direct, index, quadrant, other_mine = self.get_mine_xy()
        ...
        for key, value in self.mine_status.items():
            try:
                index = self.ori_mines.index(key)
            except ValueError:
                continue
            if '它' in value or '假' in value:
                self.quadrant_mines.pop(index)
                self.ori_mines.pop(index)
        ...
        if len(self.quadrant_mines) == 0:
            ...
            if len(self.ori_mines) == before_delete_num:
                # 看看下一个宝藏是我方真宝藏就直接撞。不识别
                if self.mine_status.get(self.ori_mines[0], "不知道") == '我真':
                    return True
                # 没有任何变化
                return False
            global obs
    ...
    def get_mine_xy(self):
        index = len(self.quadrant_mines) - len(self.paths)
        # 找到小车当前位置和朝向
        now_xy, car_direct, quadrant = self.quadrant_mines[index]
        # 找到这个宝藏的xy坐标
        mine_x, mine_y = self.ori_mines[index]
        # 同象限另一个宝藏、对称宝藏、对称宝藏的同象限宝藏。如果是None就表示已经没了这个宝藏
        same_qua = self.find_same_qua((mine_x, mine_y))

        # 对称宝藏
        sym_mine = (20 - mine_x, 20 - mine_y)
        if same_qua is not None:
            qua_sym_mine = (20 - same_qua[0], 20 - same_qua[1])
        else:
            qua_sym_mine = None
        other_mine = [same_qua, sym_mine, qua_sym_mine]

        return mine_x, mine_y, now_xy, car_direct, index, quadrant, other_mine
    ...
    def find_same_qua(self, mine_xy):
        mine_index = self.ori_mines.index(mine_xy)
        qua_num = self.quadrant_mines[mine_index][-1]
        same_qua = None
        for i in range(len(self.quadrant_mines)):
            if i != mine_index and self.quadrant_mines[i][-1] == qua_num:
                same_qua = self.ori_mines[i]
        return same_qua

    def process_mine_status(self, mine_x, mine_y, mine_result, other_mine):
        if self.our_color == 'red':
            if mine_result == 0:
                self.mine_status[(mine_x, mine_y)] = '它假'
                if other_mine[0] is not None:
                    if len(self.mine_status.get(other_mine[0], "不知道")) != 2:
                        self.mine_status[other_mine[0]] = "我"  # 同象限
                    if len(self.mine_status.get(other_mine[2], "不知道")) != 2:
                        self.mine_status[other_mine[2]] = '它'  # 同象限的对称
                self.mine_status[other_mine[1]] = '我假'  # 对称

            elif mine_result == 1:
                self.mine_status[(mine_x, mine_y)] = '它真'
                if other_mine[0] is not None:
                    if len(self.mine_status.get(other_mine[0], "不知道")) != 2:
                        self.mine_status[other_mine[0]] = "我"  # 同象限
                    if len(self.mine_status.get(other_mine[2], "不知道")) != 2:
                        self.mine_status[other_mine[2]] = '它'  # 同象限的对称
                self.mine_status[other_mine[1]] = '我真'  # 对称
            elif mine_result == 2:
                self.mine_status[(mine_x, mine_y)] = '我假'
                if other_mine[0] is not None:
                    if len(self.mine_status.get(other_mine[0], "不知道")) != 2:
                        self.mine_status[other_mine[0]] = "它"  # 同象限
                    if len(self.mine_status.get(other_mine[2], "不知道")) != 2:
                        self.mine_status[other_mine[2]] = '我'  # 同象限的对称
                self.mine_status[other_mine[1]] = '它假'  # 对称
            else:
                self.mine_status[(mine_x, mine_y)] = '我真'
                if other_mine[0] is not None:
                    if len(self.mine_status.get(other_mine[0], "不知道")) != 2:
                        self.mine_status[other_mine[0]] = "它"  # 同象限
                    if len(self.mine_status.get(other_mine[2], "不知道")) != 2:
                        self.mine_status[other_mine[2]] = '我'  # 同象限的对称
                self.mine_status[other_mine[1]] = '它真'  # 对称
        else:
            if mine_result == 0:
                self.mine_status[(mine_x, mine_y)] = '我假'
                if other_mine[0] is not None:
    ...
    def update(self, quadrant_opt: bool = True, mine=None):
        ...
        mine_x, mine_y, now_xy, car_direct, index, quadrant, other_mine = self.get_mine_xy()
        if quadrant_opt:
            # 更新一下宝藏状态：记录下已知的宝藏的真假或所属队伍
            self.process_mine_status(mine_x, mine_y, mine, other_mine)
